<x-app-layout>
    ...
    >
        <x-slot:actions>
            ...
            <x-premium.button 
                href="{{ route('guests.export', $invitation) }}" 
                variant="outline"
            >
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            >
                ...
                >
                    ...
                    <div class="flex items-center gap-2">
                        <form method="POST" action="{{ route('guests.bulk-mark-whatsapp', $invitation) }}" class="inline">
                            @csrf
                        ...
                        <form method="POST" action="{{ route('guests.bulk-delete', $invitation) }}" class="inline" onsubmit="return confirm('Delete selected guests?')">
                            @csrf
                            @method('DELETE')
                            <template x-for="id in selectedGuests" :key="id">
                                <input type="hidden" name="guest_ids[]" :value="id">
                                ...
                                <tr>
                                        ...
                                        @if($guest->whatsapp_number)
                                            ...
                                            @else
                                                <a 
                                                    ...
                                                    onclick="fetch('{{ route('guests.whatsapp-link', [$invitation, $guest]) }}', { method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' } })"
                                                >
                                    ...
                                    <td>
                                        <x-premium.dropdown-action>
                                            ...
                                            <x-premium.dropdown-item 
                                                :href="route('guests.destroy', [$invitation, $guest])" 
                                                method="DELETE"
                                                icon="trash" 
                                                :danger="true"
    ...
    <x-premium.modal name="add-guest" title="Add New Guest" maxWidth="lg">
        <form method="POST" action="{{ route('guests.store', $invitation) }}" class="space-y-4">
            @csrf
    ...
    <x-premium.modal name="import-guests" title="Import Guests from CSV" maxWidth="lg">
        <form method="POST" action="{{ route('guests.import', $invitation) }}" enctype="multipart/form-data" class="space-y-4">
            @csrf
    ...
    @foreach($guests as $guest)
        <x-premium.modal name="edit-guest-{{ $guest->id }}" title="Edit Guest" maxWidth="lg">
            <form method="POST" action="{{ route('guests.update', [$invitation, $guest]) }}" class="space-y-4">
                @csrf
                @method('PUT')
